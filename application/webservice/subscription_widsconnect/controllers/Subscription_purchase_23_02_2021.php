...
class Subscription_purchase extends Cit_Controller
{
     ...
     public function add_user_influencer_revenue($input_params = array())
    {
        ...
        try
        {
             ...
             if (isset($input_params["expiry_date"]))
            {
                 ...
                 if($input_params["product_id"]=='com.app.Rebound.7days')
                {
                    //$now = strtotime($input_params["expiry_date"]);
                    //$start_date = $now;
                   // $end_date = strtotime("+7 day", $start_date);
                    $expire_date= date('Y-m-d h:i:s',strtotime("+7 day"));
                    $params_arr["expiry_date"] = $expire_date;
    ...
    public function subscription_purchase_android($input_params = array())
    {
      ...
      public function add_user_influencer_revenue_android($input_params = array())
    {
        ...
        try
        {
            ...
            if (isset($input_params["subscription_id"]))
            {
                $params_arr["product_id"] = $input_params["subscription_id"];
            }

             if (isset($input_params["expiry_date_v1"]))
            {
                 $params_arr["expiry_date"] = $input_params["expiry_date_v1"];
                
                
            }
           
            $this->block_result = $this->subscription_purchase_model->add_user_influencer_revenue_android($params_arr);
        }
        catch(Exception $e)
        {
            $success = 0;
        ...
        $input_params["add_user_influencer_revenue_android"] = $this->block_result["data"];
        $input_params = $this->wsresponse->assignSingleRecord($input_params, $this->block_result["data"]);
