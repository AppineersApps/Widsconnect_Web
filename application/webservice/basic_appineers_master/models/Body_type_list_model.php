...
class Body_type_list_model extends CI_Model
{
    /**
    ...
    public function body_type_list()
    {
       // print_r("Text");exit;
            ...
            $this->db->from("body_types");
            $this->db->select('iBodytypeId AS BodytypeId');
            $this->db->select('vBodyType AS BodyType');

            $this->db->order_by("vBodyType", "asc");
     ...
     public function get_body_type_list($arrResult)
    {
        ...
        try
        {
            ...
            if(false == empty($arrResult['BodytypeId']))
            {
              $this->db->where("iBodytypeId = '".$arrResult['BodytypeId']."'"); 
            }
