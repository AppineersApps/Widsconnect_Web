...
class Blocked_user_model extends CI_Model
{
    ...
    public function if_blocked($user_id = '', $receiver_id = '')
    {
        try
        {
            ...
            $this->db->select("bu.iUserId AS bu_blocked_to");
            $this->db->select("bu.iBlockUserId AS bu_blocked_by");
            $this->db->where("(bu.dtAddedAt IS NOT NULL AND bu.dtAddedAt <> '')", FALSE, FALSE);
            $this->db->where("(bu.iUserId = ".$user_id." AND bu.iBlockUserId = ".$receiver_id.") OR (bu.iUserId = ".$receiver_id." AND bu.iBlockUserId = ".$user_id.")", FALSE, FALSE);
