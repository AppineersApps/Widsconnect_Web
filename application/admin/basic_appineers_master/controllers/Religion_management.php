...
/**
 * Description of Religion Management Controller
 *
 * @category admin
 *
 ...
 *
 * @module Religion Management
 *
 * @class Religion_management.php
 *
 * @path application\admin\basic_appineers_master\controllers\Religion_management.php
 *
 * @version 4.4
 *
 * @author CIT Dev Team
...
class Religion_management extends Cit_Controller
{
    /**
    ...
    public function __construct()
    {
        ...
        $this->load->model('religion_management_model');
        $this->folder_name = "basic_appineers_master";
        $this->module_name = "religion_management";
        ...
        $this->dropdown_arr = array(
            ...
            "i_religion_status" => array(
                "type" => "enum",
                "default" => "Yes",
                "values" => array(
                    array(
                        'id' => 'Active',
                        'val' => $this->lang->line('RELIGION_MANAGEMENT_ACTIVE')
                    ),
                    array(
                        'id' => 'Inactive',
                        'val' => $this->lang->line('RELIGION_MANAGEMENT_INACTIVE')
                    )
                )
            )
        );
    ...
    public function index()
    {
        ...
        try
        {
            ...
            {
                $access_list = array(
                    "religion_management_list",
                    "religion_management_view",
                    "religion_management_add",
                    "religion_management_update",
                    "religion_management_delete",
                );
            ...
            else
            {
                ...
                list($list_access, $view_access, $add_access, $edit_access, $del_access, $expo_access, $print_access) = $this->filter->getModuleWiseAccess("religion_management", $access_list, TRUE, TRUE);
            }
            if (!$list_access)
            {
                throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_VIEW_THIS_PAGE_C46_C46_C33'));
            }
            $enc_loc_module = $this->general->getMD5EncryptString("ListPrefer", "religion_management");
            ...
            $status_label = array(
                'js_lang_label.RELIGION_MANAGEMENT_ACTIVE',
                'js_lang_label.RELIGION_MANAGEMENT_INACTIVE',
            );

            $list_config = $this->religion_management_model->getListConfiguration();
            $this->processConfiguration($list_config, $add_access, $edit_access, TRUE);
            $this->general->trackModuleNavigation("Module", "List", "Viewed", $this->mod_enc_url["index"], "religion_management");
            ...
            $render_arr = array(
                ...
                'default_filters' => $this->religion_management_model->default_filters,
            );
            ...
            {
                $this->loadView("religion_management_index");
            }
        }
        catch(Exception $e)
        {
    ...
    public function listing()
    {
        ...
        $list_config = $this->religion_management_model->getListConfiguration();
        $form_config = $this->religion_management_model->getFormConfiguration();
        $extra_cond = $this->religion_management_model->extra_cond;
        $groupby_cond = $this->religion_management_model->groupby_cond;
        $having_cond = $this->religion_management_model->having_cond;
        $orderby_cond = $this->religion_management_model->orderby_cond;
        ...
        $data_recs = $this->religion_management_model->getListingData($data_config);
        $data_recs['no_records_msg'] = $this->general->processMessageLabel('ACTION_NO_RELIGION_MANAGEMENT_DATA_FOUND_C46_C46_C33');
    ...
    public function export()
    {
        if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
        {
            $this->filter->checkAccessCapability("religion_management_export");
        }
        else
        {
            $this->filter->getModuleWiseAccess("religion_management", "Export", TRUE);
        }
        $params_arr = $this->params_arr;
        $page = $params_arr['page'];
        ...
        $list_config = $this->religion_management_model->getListConfiguration();
        $form_config = $this->religion_management_model->getFormConfiguration();
        $table_name = $this->religion_management_model->table_name;
        $table_alias = $this->religion_management_modeltable_alias;
        $primary_key = $this->religion_management_model->primary_key;
        $extra_cond = $this->religion_management_model->extra_cond;
        $groupby_cond = $this->religion_management_model->groupby_cond;
        $having_cond = $this->religion_management_model->having_cond;
        $orderby_cond = $this->religion_management_model->orderby_cond;
        ...
        $db_recs = $this->religion_management_model->getExportData($export_config);
        ...
        $misc_info['heading'] = $this->lang->line('RELIGION_MANAGEMENT_RELIGION_MANAGEMENT');
        $misc_info['filename'] = "religion_management_records_".count($db_recs);
        ...
        if ($export_type == 'pdf')
        {
            ...
            {
                $pdf->setModule("religion_management_model");
            }
        ...
        try
        {
            ...
            if ($mode == "Update")
            {
                ...
                else
                {
                    ...
                    list($list_access, $view_access, $edit_access, $del_access, $print_access) = $this->filter->getModuleWiseAccess("religion_management", $access_list, TRUE, TRUE);
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    list($list_access, $interestscess) = $this->filter->getModuleWiseAccess("religion_management", $access_list, TRUE, TRUE);
                }
            ...
            if ($mode == 'Update')
            {
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowEdit", "religion_management"), $this->session->userdata('iAdminId'));
                $data_arr = $this->religion_management_model->getData(intval($id));
                ...
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "religion_management", $recName);
            }
            else
            {
                $recName = '';
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowAdd", "religion_management"), $this->session->userdata('iAdminId'));
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "religion_management");
            }

            $opt_arr = $img_html = $auto_arr = $config_arr = array();

            $form_config = $this->religion_management_model->getFormConfiguration($config_arr);
                ...
                foreach ($form_config as $key => $val)
                {
                    ...
                    if ($val['function'] != "")
                    {
                        ...
                        elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                        {
                            ...
                            if (method_exists($this->religion_management_model, $phpfunc))
                            {
                                $tmpdata = $this->religion_management_model->$phpfunc($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                            }
                        }
                    ...
                    if ($val['field_status'] != "")
                    {
                        ...
                        elseif ($status_type == "function")
                        {
                            ...
                            elseif (substr($fd_callback, 0, 7) == 'model::' && substr($fd_callback, 7) !== FALSE)
                            {
                                ...
                                if (method_exists($this->religion_management_model, $fd_callback))
                                {
                                    $fd_status = $this->religion_management_model->$fd_callback($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                                }
                            }
            ...
            else
            {
                if ($mode == "Update")
                {
                    ...
                    {
                        $this->loadView("religion_management_add");
                    }
                    else
                    {
                        $this->loadView("religion_management_add_view");
                    }
                }
                else
                {
    ...
    public function addAction()
    {
        ...
        try
        {
            ...
            if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
            {
                if ($mode == "Update")
                {
                    $add_edit_access = $this->filter->checkAccessCapability("religion_management_update", TRUE);
                }
                else
                {
                    $add_edit_access = $this->filter->checkAccessCapability("religion_management_add", TRUE);
                }
            }
            else
            {
                $add_edit_access = $this->filter->getModuleWiseAccess("religion_management", $mode, TRUE, TRUE);
            }
            if (!$add_edit_access)
            {
                if ($mode == "Update")
          ...
          //  $i_religion_image = $params_arr["i_religion_image"];
            $i_religion_name = $params_arr["i_religion_name"];
            $i_religion_status = $params_arr["i_religion_status"];
            //$i_added_at = "NOW()";
            $i_added_at =$params_arr["i_added_at"];

            $data = $save_data_arr = $file_data = array();
           // $data["vreligionImage"] = $i_religion_image;
            $data["vReligionName"] = $i_religion_name;
            $data["eStatus"] = $i_religion_status;

            $data["dtAddedAt"] = $this->filter->formatActionData($i_added_at, $form_config["i_added_at"]);

        //    $save_data_arr["i_religion_image"] = $data["vreligionImage"];
            $save_data_arr["i_religion_name"] = $data["vReligionName"];
            $save_data_arr["i_religion_status"] = $data["eStatus"];
            $save_data_arr["i_added_at"] = $data["dtAddedAt"];
            $strCheck = $this->checkUniqueInterest($id);
            if ($mode == 'Add')
            {
              
                $id = $this->religion_management_model->insert($data);
                if (intval($id) > 0)
                {
                    $save_data_arr["iReligionId"] = $data["iReligionId"] = $id;
                ...
                $track_cond = $this->db->protect("i.iReligionId")." = ".$this->db->escape($id);
                $switch_combo = $this->religion_management_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Added", $this->mod_enc_url["add"], "religion_management", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            elseif ($mode == 'Update' && false== $strCheck)
            {
                $res = $this->religion_management_model->update($data, intval($id));
                if (intval($res) > 0)
                {
                    $save_data_arr["iReligionId"] = $data["iReligionId"] = $id;
                ...
                $switch_combo = $this->religion_management_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Modified", $this->mod_enc_url["add"], "religion_management", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            $ret_arr['id'] = $id;
            $ret_arr['mode'] = $mode;
             if(true==$strCheck)
           ...
           // $file_data["i_religion_image"]["file_name"] = $i_religion_image;
           // $file_data["i_religion_image"]["old_file_name"] = $params_arr["old_i_religion_image"];
           // $file_data["i_religion_image"]["unique_name"] = "i_religion_image";
          //  $file_data["i_religion_image"]["primary_key"] = "iReligionId";
    ...
    public function inlineEditAction()
    {
        ...
        if ($all_row_selected == "true" && in_array($operartor, array("del", "status")) && false==$strCheck)
        {
            ...
            $config_arr['list_config'] = $this->religion_management_model->getListConfiguration();
            $config_arr['form_config'] = $this->religion_management_model->getFormConfiguration();
            $config_arr['table_name'] = $this->religion_management_model->table_name;
            $config_arr['table_alias'] = $this->religion_management_model->table_alias;
        ...
        if ($search_alias == "Yes")
        {
            $primary_field = $this->religion_management_model->table_alias.".".$this->religion_management_model->primary_key;
        }
        else
        {
            $primary_field = $this->religion_management_model->primary_key;
        }
        ...
        try
        {
            switch ($operartor)
            {
                case 'del':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $del_access = $this->filter->checkAccessCapability("religion_management_delete", TRUE);
                    }
                    else
                    {
                        $del_access = $this->filter->getModuleWiseAccess("religion_management", "Delete", TRUE, TRUE);
                    }
                    if (!$del_access)
                    {
                    ...
                    if (is_array($primary_ids))
                    {
                        $main_cond = $this->db->protect("i.iInterestsId")." IN ('".implode("','", $primary_ids)."')";
                    }
                    else
                    {
                        $main_cond = $this->db->protect("i.iInterestsId")." = ".$this->db->escape($primary_ids);
                    }
                    if(false==$strCheck)
                    {
                       $success = $this->religion_management_model->delete($extra_cond, $search_alias, $search_join);  
                    }else{
                        $success = 1;
                    }
                    $main_data = $this->religion_management_model->getData($main_cond, array('i.vInterestsImage'));
                    $success = $this->religion_management_model->delete($extra_cond, $search_alias, $search_join);
                    if (!$success)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_FAILURE_IN_DELETION_THIS_RECORD_C46_C46_C33'));
                ...
                case 'edit':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $edit_access = $this->filter->checkAccessCapability("religion_management_update", TRUE);
                    }
                    else
                    {
                        $edit_access = $this->filter->getModuleWiseAccess("religion_management", "Update", TRUE, TRUE);
                    }
                    if (!$edit_access)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_MODIFY_THESE_DETAILS_C46_C46_C33'));
                    ...
                    $list_config = $this->religion_management_model->getListConfiguration($post_name);
                    $form_config = $this->religion_management_model->getFormConfiguration($list_config['source_field']);
                    if (!is_array($form_config) || count($form_config) == 0)
                    {
                    ...
                    $db_rec_arr = $this->religion_management_model->getData(intval($primary_ids));

                    $data_arr[$field_name] = $post_val;
                    if(false==$strCheck)
                    {
                      $success = $this->religion_management_model->update($data_arr, intval($primary_ids)); 
                    }else{
                        $success = 1;
                    }
                    $success = $this->religion_management_model->update($data_arr, intval($primary_ids));
                ...
                case 'status':
                    ...
                    {
                        $field_name = $this->religion_management_model->table_alias.".eStatus";
                    }
                    else
                    {
                    ...
                    //$success = $this->religion_management_model->update($data_arr, $extra_cond, $search_alias, $search_join);
                    if (!$success)
                    {
                     ...
                     if(false==$strCheck)
                     {
                        ...
                        $message = "This Interest is already in used. So can not be Deleted or Inactivated"; 
                     }
            ...
            if ($mode == "Update")
            {
                ...
                if ($form_config["type"] == "file")
                {
                    ...
                    $file_data[$unique_name]["primary_key"] = "iInterestsId";
    ...
    public function getSourceOptions($name = '', $mode = 'Add', $id = '', $data = array(), $extra = '', $rtype = 'records')
    {
        ...
        switch ($type)
        {
            ...
            case 'table':
                ...
                {
                    $combo_config['main_table'] = $this->religion_management_model->table_name;
            ...
            case 'phpfn':
                ...
                elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                {
                    ...
                    if (method_exists($this->religion_management_model, $phpfunc))
                    {
                        $data_arr = $this->religion_management_model->$phpfunc($data[$name], $mode, $id, $data, $parent_src, $this->term);
                    }
                }
                elseif (method_exists($this->general, $phpfunc))
                {
    ...
    public function getSelfSwitchTo()
    {
        ...
        $switchto_fields = $this->religion_management_model->switchto_fields;
        $extra_cond = $this->religion_management_model->extra_cond;
        ...
        $switch_arr = $this->religion_management_model->getSwitchTo($extra_cond);
    ...
    public function getListOptions()
    {
        ...
        $config_arr = $this->religion_management_model->getListConfiguration($alias_name);
    ...
    public function downloadListFile()
    {
        ...
        if (is_array($config_arr) && count($config_arr) > 0)
        {
            ...
            $data_arr = $this->religion_management_model->getData(intval($id), $fields, '', '', '', 'Yes');
    ...
    public function uploadFormFile()
    {
        ...
        $config_arr = $this->religion_management_model->getFormConfiguration($unique_name);
    ...
    public function deleteFormFile()
    {
        ...
        try
        {
            ...
            else
            {
                ...
                $res = $this->religion_management_model->update($update_arr, intval($id));
                if (!$res)
                {
