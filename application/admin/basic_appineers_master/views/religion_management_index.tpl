...
<div class="module-list-container">
    <%include file="religion_management_index_strip.tpl"%>
...
<%javascript%>
    ...
    js_col_name_json = [
    {
        "name": "i_religion_name",
        "label": "<%$list_config['i_religion_name']['label_lang']%>"
    },
    {
        "name": "i_religion_status",
        "label": "<%$list_config['i_religion_status']['label_lang']%>"
    },
    {
        "name": "i_added_at",
    ...
    {
        "name": "i_religion_name",
        "index": "i_religion_name",
        "label": "<%$list_config['i_religion_name']['label_lang']%>",
        "labelClass": "header-align-left",
        "resizable": true,
        "width": "<%$list_config['i_religion_name']['width']%>",
        "search": <%if $list_config['i_religion_name']['search'] eq 'No' %>false<%else%>true<%/if%>,
        "export": <%if $list_config['i_religion_name']['export'] eq 'No' %>false<%else%>true<%/if%>,
        "sortable": <%if $list_config['i_religion_name']['sortable'] eq 'No' %>false<%else%>true<%/if%>,
        "hidden": <%if $list_config['i_religion_name']['hidden'] eq 'Yes' %>true<%else%>false<%/if%>,
        "hideme": <%if $list_config['i_religion_name']['hideme'] eq 'Yes' %>true<%else%>false<%/if%>,
        "addable": <%if $list_config['i_religion_name']['addable'] eq 'Yes' %>true<%else%>false<%/if%>,
        "editable": <%if $list_config['i_religion_name']['editable'] eq 'Yes' %>true<%else%>false<%/if%>,
        "align": "left",
        "edittype": "text",
        "editrules": {
            "required": true,
            "infoArr": {
                "required": {
                    "message": ci_js_validation_message(js_lang_label.GENERIC_PLEASE_ENTER_A_VALUE_FOR_THE__C35FIELD_C35_FIELD_C46 ,"#FIELD#",js_lang_label.RELIGION_MANAGEMENT_NAME)
                }
            }
        },
        "searchoptions": {
            "attr": {
                ...
                "aria-module-name": "religion_management",
                "aria-unique-name": "i_religion_name",
                "autocomplete": "off"
            },
            "sopt": strSearchOpts,
            "searchhidden": <%if $list_config['i_religion_name']['search'] eq 'Yes' %>true<%else%>false<%/if%>
        },
        "editoptions": {
            "aria-grid-id": el_tpl_settings.main_grid_id,
            "aria-module-name": "religion_management",
        ...
        "ctrl_type": "textbox",
        "default_value": "<%$list_config['i_religion_name']['default']%>",
        "filterSopt": "bw",
    ...
    {
        "name": "i_religion_status",
        "index": "i_religion_status",
        "label": "<%$list_config['i_religion_status']['label_lang']%>",
        "labelClass": "header-align-center",
        "resizable": true,
        "width": "<%$list_config['i_religion_status']['width']%>",
        "search": <%if $list_config['i_religion_status']['search'] eq 'No' %>false<%else%>true<%/if%>,
        "export": <%if $list_config['i_religion_status']['export'] eq 'No' %>false<%else%>true<%/if%>,
        "sortable": <%if $list_config['i_religion_status']['sortable'] eq 'No' %>false<%else%>true<%/if%>,
        "hidden": <%if $list_config['i_religion_status']['hidden'] eq 'Yes' %>true<%else%>false<%/if%>,
        "hideme": <%if $list_config['i_religion_status']['hideme'] eq 'Yes' %>true<%else%>false<%/if%>,
        "addable": <%if $list_config['i_religion_status']['addable'] eq 'Yes' %>true<%else%>false<%/if%>,
        "editable": <%if $list_config['i_religion_status']['editable'] eq 'Yes' %>true<%else%>false<%/if%>,
        "align": "center",
        "edittype": "select",
        "editrules": {
            "required": true,
            "infoArr": {
                "required": {
                    "message": ci_js_validation_message(js_lang_label.GENERIC_PLEASE_ENTER_A_VALUE_FOR_THE__C35FIELD_C35_FIELD_C46 ,"#FIELD#",js_lang_label.RELIGION_MANAGEMENT_STATUS)
                }
            }
        },
        "searchoptions": {
            "attr": {
                "aria-grid-id": el_tpl_settings.main_grid_id,
                "aria-module-name": "religion_management",
                "aria-unique-name": "i_religion_status",
                "autocomplete": "off",
                "data-placeholder": " ",
            ...
            "sopt": intSearchOpts,
            "searchhidden": <%if $list_config['i_religion_status']['search'] eq 'Yes' %>true<%else%>false<%/if%>,
            "dataUrl": <%if $count_arr["i_religion_status"]["json"] eq "Yes" %>false<%else%>'<%$admin_url%><%$mod_enc_url["get_list_options"]%>?alias_name=i_religion_status&mode=<%$mod_enc_mode["Search"]%>&rformat=html<%$extra_qstr%>'<%/if%>,
            "value": <%if $count_arr["i_religion_status"]["json"] eq "Yes" %>$.parseJSON('<%$count_arr["i_religion_status"]["data"]|@addslashes%>')<%else%>null<%/if%>,
            "dataInit": <%if $count_arr['i_religion_status']['ajax'] eq 'Yes' %>initSearchGridAjaxChosenEvent<%else%>initGridChosenEvent<%/if%>,
            "ajaxCall": '<%if $count_arr["i_religion_status"]["ajax"] eq "Yes" %>ajax-call<%/if%>',
            "multiple": true
        },
        "editoptions": {
            "aria-grid-id": el_tpl_settings.main_grid_id,
            "aria-module-name": "religion_management",
            "aria-unique-name": "i_religion_status",
            "dataUrl": '<%$admin_url%><%$mod_enc_url["get_list_options"]%>?alias_name=i_religion_status&mode=<%$mod_enc_mode["Update"]%>&rformat=html<%$extra_qstr%>',
            "dataInit": <%if $count_arr['i_religion_status']['ajax'] eq 'Yes' %>initEditGridAjaxChosenEvent<%else%>initGridChosenEvent<%/if%>,
            "ajaxCall": '<%if $count_arr["i_religion_status"] eq "Yes" %>ajax-call<%/if%>',
            "data-placeholder": "<%$this->general->parseLabelMessage('GENERIC_PLEASE_SELECT__C35FIELD_C35' ,'#FIELD#', 'RELIGION_MANAGEMENT_STATUS')%>",
            "class": "inline-edit-row chosen-select"
        },
        "ctrl_type": "dropdown",
        "default_value": "<%$list_config['i_religion_status']['default']%>",
        "filterSopt": "in",
        "stype": "select"
    },
    {
